...
class AppStore extends Component {
  ...
  changeTabIdFunction = changedId => {
    this.setState({activeTab: changedId})
  }

  inputChangeFunction = event => {
    const inputValue = event.target.value
    const searchFilteredList = appsList.filter(eachItem =>
      eachItem.appName.toLowerCase().includes(inputValue.toLowerCase()),
    )
    this.setState({searchValue: inputValue, searchFilteredList})
  }

  render() {
    const {searchValue, activeTab, searchFilteredList} = this.state
    ...
    return (
      <div className="css-bg-container">
        ...
        <div className="css-search-input-container">
          <input
            type="search"
            className="css-search-itself"
            placeholder="Search"
            value={searchValue}
            onChange={this.inputChangeFunction}
          />
          <img
            src="https://assets.ccbp.in/frontend/react-js/app-store/app-store-search-img.png"
            alt="search icon"
            className="css-searchIcon-itself"
          />
        </div>
        <ul className="css-tablist-container">
          {tabsList.map(item => (
            <TabItem
              tabItem={item}
              activeTab={activeTab}
              onclickFunction={this.changeTabIdFunction}
              key={item.id}
            />
          ))}
        </ul>
        <ul className="css-appList-container">
